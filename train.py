...
def train(args):
    ...
    # model.load_state_dict(torch.load('output/53_0.pth', map_location=device), strict=True)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch == 0 or val_epoch_rank >= np.max(val_ranks):
            torch.save(model.state_dict(), args.output_folder + '62_resnext50_32x4d_' + args.fold + '.pth')
            num_not_improve = 0
        else:
            num_not_improve += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tp_train', default='fold/5/train_filenames.txt')
    parser.add_argument('--tp_val', default='fold/5/val_filenames.txt')
